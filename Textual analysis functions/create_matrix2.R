create_matrix2 <- function (textColumns, language = "english", minDocFreq = 1, 
                            ...
                            weighting = weightTf) 
{
  ...
  tokenize_ngrams <- function(x, n = ngramLength) return(rownames(as.data.frame(unclass(textcnt(as.character(x), 
                                                                                                method = "string", n = n)))))
